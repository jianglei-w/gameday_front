<template>
  <div class="container">
    <el-card>
      ...
      <!-- 添加题目按钮 -->
      <el-button class="define" @click="topicDialogVisible = true"
        >新建题目组</el-button
      >
      <el-button class="define" @click="topicListDrawer = true"
        >题目列表</el-button
      >
      ...
      <div class="topicGroup">
        <el-card v-for="(item, index) in getTopicGroup" :key="item.id">
          ...
          <p>
            题数:<span>{{
              item.Questions.length ? item.Questions.length : 0
            }}</span
            >题
          </p>
    ...
    >
      <el-form
        ...
        label-width="100px"
      >
        <el-form-item label="题目组名称" prop="group_name">
          <el-input v-model="topicGroupList.group_name"></el-input>
        </el-form-item>
    ...
    <el-drawer title="题目组管理" :visible.sync="topicGroupDrawer" size="50%">
      <h1>题目列表</h1>
      <el-table :data="oneTopicGroupData" height="450" border>
        <el-table-column type="index" label="#"> </el-table-column>
        <el-table-column prop="title" label="题目名称"> </el-table-column>
        <el-table-column prop="level" label="难度等级"> </el-table-column>
        <el-table-column prop="answer" label="题目答案"> </el-table-column>
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button class="empty" @click="deleteGroupSubject(scope.row)"
              >移除</el-button
            >
          </template>
        </el-table-column>
      </el-table>
      <el-button class="define btn" @click="topicGroupDrawer = false"
        >关闭</el-button
      >
      <el-button class="define btn" @click="addTopicDialogVisible = true"
        >新增题目</el-button
      >
      <el-button class="empty btn" @click="deleteTopicGroup"
        >删除题目组</el-button
      >
    </el-drawer>
    <!-- 所有题目列表的抽屉 -->
    <el-drawer :visible.sync="topicListDrawer" size="55%" title="题目列表">
      <el-table :data="topicdata" height="450" border class="ranking">
        <el-table-column type="index" label="#"> </el-table-column>
        <el-table-column prop="title" label="题目名称"></el-table-column>
        <el-table-column prop="level" label="难度等级"></el-table-column>
        <el-table-column prop="answer" label="题目答案"></el-table-column>
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button class="define" @click="modifyTopic(scope.row)"
              >编辑</el-button
            >
            <el-button class="empty" @click="deleteTopic(scope.row)"
              >删除</el-button
            >
          </template>
        </el-table-column>
    ...
    <el-drawer title="编辑题目" :visible.sync="TextDialogVisible" size="50%">
      ...
      >
        <el-form-item label="题目名称:" prop="headline">
          <el-input v-model="upload.title"></el-input>
        </el-form-item>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button class="define" @click="TextDialogVisibleEdit = false"
          >确 定</el-button
        >
      </span>
    </el-dialog>
...
export default {
  data() {
    return {
      ...
      topicGroupRules: {
        ...
        groupname: [
          { required: true, message: '请输入题目组名称', trigger: 'blur' },
        ],
        subject_list: [
  ...
  methods: {
    ...
    async getTopicList() {
      ...
      this.topicdata = res.data.reverse()
    },
    // 获取所有题目组
    async showGroup() {
      const { data: res } = await this.$http.get(
      ...
      this.getTopicGroup = res.data
      // console.log(this.getTopicGroup.Questions[0].length)
    },
    // 新建题目组
    newTopicGroup() {
    ...
    topicAdministration(index) {
      // console.log(this.getTopicGroup, 'index', index)
      // 获取单个题目组里的题目id
      ...
      // this.removeTopic.question_id = this.getTopicGroup[index].ID
      // 删除题目的id
    ...
    async getOneTopicGroupData() {
      const { data: res } = await this.$http.get(
        `oneGroup?group_id=${this.onegroup.group_id}`
      )
      if (res.status !== 0) {
        return this.$message.error('获取该题目组的题目失败')
      }
      this.oneTopicGroupData = res.data.Questions
    },
    // 移除题目组里的题目
    deleteGroupSubject(scope) {
      this.$confirm('确认要移除该题目吗?', '提示', {
        ...
        .then(async () => {
          this.removeTopic.question_id = scope.ID
          this.removeTopic.subject_list = parseInt(
            this.removeTopic.subject_list,
            10
          )
          this.removeTopic.group_id = parseInt(this.removeTopic.group_id, 10)
          const { data: res } = await this.$http.post(
            'deleteGroupSubject',
            this.removeTopic
          )
    ...
    async deleteTopic(scope) {
      this.$confirm('确定要删除该题目吗?', '提示', {
        ...
        .then(async () => {
          ...
          deleteTopicParameter.id = scope.ID
          const { data: res } = await this.$http.post(
            'deleteQuestion',
            deleteTopicParameter
          )
          if (res.status !== 0) {
    ...
    async uploadSubject() {
      ...
      const { data: res } = await this.$http.post('updateQuestion', this.upload)
