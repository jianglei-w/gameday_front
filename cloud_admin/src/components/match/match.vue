<template>
  <div>
    <el-card>
      ...
      <el-row :gutter="30">
        <el-col :span="12" v-for="(item, index) in gameDataList" :key="item.ID">
          <el-card>
            <div class="cardInfo">
              <h1>{{ item.game_name }}</h1>
              <p>
                比赛ID：<span>{{ item.ID }}</span>
              </p>
              <p>
                比赛人数：<span>{{ item.people }}</span
                >人
              </p>
              <p>
                比赛状态：<span v-show="item.game_status == 0">未开始</span
                ><span v-show="item.game_status == 1">进行中</span
                ><span v-show="item.game_status == 2">暂停中</span>
                <span v-show="item.game_status == 3">已完成</span>
              </p>
              <p>
                比赛创建时间：<span>{{ item.CreatedAt }}</span>
              </p>
              <p>
                题目组：<span>{{ item.group.group_name }}</span>
              </p>
              <div class="btnStatus">
                <el-button
                  type="primary"
                  class="define"
                  @click="setGame(item.ID)"
                  >选择题组</el-button
                >
                <el-button
                  type="primary"
                  class="define"
                  @click="adminGame(index, item.ID)"
                  >比赛管理</el-button
                >
                <el-button
                  type="primary"
                  class="ranking define"
                  @click="rankingList(index)"
                  >排行榜</el-button
                >
              </div>
            </div>
    ...
    <!-- 比赛管理的抽屉 -->
    <el-drawer title="审核用户名" :visible.sync="GameDrawer" size="55%">
      <el-table :data="gameUserData" height="85%" border style="width: 100%">
        <!-- <el-table-column prop="game_name" label="比赛名称"> </el-table-column> -->
        <el-table-column prop="hashcode" label="选手hash值"> </el-table-column>
          ...
          <template slot-scope="scope">
            <div class="scopeBtn">
              ...
              <el-button
                ...
                >待审核</el-button
              >
              <el-button class="empty" size="small" v-if="scope.row.status == 3"
                >已驳回</el-button
              >
              <el-button
                class="define"
          ...
          <template slot-scope="scope">
            <div class="scopeBtn">
              <el-button
                class="define"
                size="small"
                @click="adoptUserName(scope.row)"
                >通过</el-button
              >
              <el-button
                class="empty"
                size="small"
                @click="rejectUserName(scope.row)"
                >驳回</el-button
              >
            </div>
          </template>
      ...
      <el-button type="danger" class="btnGame empty" @click="deleteGame"
        >删除比赛</el-button
      >
      <el-button type="warning" class="btnGame btnyellow" @click="suspendGame"
        >暂停比赛</el-button
      >
      <el-button type="primary" class="btnGame define" @click="startGame"
        >开始比赛</el-button
      >
    </el-drawer>
    <!-- 排行榜的抽屉 -->
    ...
    >
      <el-table :data="rankingdata" height="90%" border class="ranking">
        <el-table-column type="index" label="排名" width="80px">
          <template slot-scope="scope">
            <img
              v-if="scope.$index == 0"
              src="../../assets/image/pm/one.png"
              alt=""
            />
            <img
              v-else-if="scope.$index == 1"
              src="../../assets/image/pm/two.png"
              alt=""
            />
            <img
              v-else-if="scope.$index == 2"
              src="../../assets/image/pm/three.png"
              alt=""
            />
            <span v-else>{{ scope.$index + 1 }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="username" label="选手名称"> </el-table-column>
        <el-table-column prop="accomplish" label="完成题目总数">
        </el-table-column>
        <el-table-column prop="score" label="总分"> </el-table-column>
      </el-table>
    </el-drawer>
    <!-- 选择题目组对话框 -->
    <el-dialog title="提示" :visible.sync="topicDialogVisible" width="30%">
      <el-form
      ...
      >
        <el-form-item label="选择题目组" prop="group_id">
          <el-select v-model="topicGroupList.group_id" placeholder="请选择">
            <el-option
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" class="define" @click="choiceTopic"
          >确 定</el-button
        >
      </span>
    </el-dialog>
...
export default {
  data() {
    return {
      ...
      topicGroupRules: {
        group_id: [
          { required: true, message: '请选择题目组', trigger: 'blur' },
        ],
      },
      // 获取所有比赛的请求参数
      getGame: {
  ...
  methods: {
    ...
    async getGameList() {
      // 1.2 发请求
      const { data: res } = await this.$http.get('/getallGame', this.getGame)
      // 1.3 判断状态码
      if (res.status !== 0) {
        return this.$message.error('获取所有比赛失败')
      }
      // res.data.all_game.reverse()
      this.gameDataList = res.data
    },
    // 1.2 获取所有的题目组
    async getGameTopicGroup() {
      const { data: res } = await this.$http.get('/showGroup') //, this.getGame)
      if (res.status !== 0) {
    ...
    adminGame(index) {
      // 赋值 删除比赛的比赛名称
      this.deGame.id = this.gameDataList[index].ID
      // 赋值 开始比赛的id
      this.staGame.id = this.gameDataList[index].ID
      // 赋值 比赛是否进行中的状态
      this.gameFlag = this.gameDataList[index].game_status
      // 审核用户名
      this.getHash.game_id = this.gameDataList[index].ID
      this.gameTimer = setInterval(async () => {
        const { data: res } = await this.$http.get(
          `getHash?game_id=${this.gameDataList[index].ID}`
        )
        if (res.status !== 0) return
        this.gameUserData = res.data.Hashes
      }, 1000)
    ...
    rankingList(index) {
      ...
      this.gameTimer = setInterval(async () => {
        // 请求参数
        const { data: res } = await this.$http.get(
          `RankList?game_id=${this.gameDataList[index].ID}`
          // this.getRankingList
        )
        if (res.status !== 0) {
        ...
        // 排序
        this.rankingdata = res.data.Hashes.sort((a, b) => {
    ...
    async deleteGame() {
      ...
      // if (this.gameFlag === 0 || this.gameFlag === 1 || this.gameFlag === 2) {
      //   return this.$message.error('比赛未完成，无法删除！')
      // }
      this.$confirm('是否删除该比赛?', '提示', {
        ...
        this.deGame.id = parseInt(this.deGame.id, 10)
        const { data: res } = await this.$http.post('deleteGame', this.deGame)
        if (res.status !== 0) {
          return this.$message.error('比赛已完成')
        }
        this.$message({
          type: 'success',
          message: '比赛已完成',
    ...
    startGame() {
      this.$confirm('是否正式开始比赛?', '提示', {
        ...
        this.staGame.id = parseInt(this.staGame.id, 10)
